...
class PlaceObjectService(object):
    ...
    def __handle_request(self, request):
        ...
        result = self.__place.execute(taskdata)
        return TaskDataServiceResponse(taskdata=taskdata, result=result)


class CheckPlacementService(object):
    SERVICE_NAME = "sutro/state/check_placement"

    def __init__(self):
        rospy.Service(self.SERVICE_NAME, TaskDataService, self.__handle_request)
        self.__check = CheckPlacement()

    def __handle_request(self, request):
        taskdata = request.taskdata
        result = self.__check.execute(taskdata)
        return TaskDataServiceResponse(taskdata=taskdata, result=result)
