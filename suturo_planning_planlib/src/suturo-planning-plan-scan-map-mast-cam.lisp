...
(defconstant +service-name+ "/suturo/manipulation/move_mastcam" "The name of the service to move the mastcam")
...
(defconstant +waiting-time-before-scan+ 100)
...
(defun scan-map-part(pan tilt)
  (if (not (roslisp:wait-for-service +service-name+ *timeout-service*))
      ...
      (progn
        (roslisp:call-service +service-name+ 'suturo_planning_manipulation-srv:MoveMastCam :pan pan :tilt tilt)
...
(def-cram-function state-scan-map ()
    (loop while T do
      ...
      (format t "Executing state scan map")
